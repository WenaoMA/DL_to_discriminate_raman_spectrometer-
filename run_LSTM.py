...
data = np.load('./data/NormalTumor_cut10.npy')
...
#0.01
model = Sequential()
model.add(Convolution1D(64,64, input_shape=(window, 1))) 
# nb_filter:Number of convolution kernels to use(dimensionality of the output)
# filter_length: The extension (spatial or temporal) of each filter.
model.add(Activation('relu'))
# model.add(Flatten()) 
model.add(Dropout(0.01)) 
model.add(LSTM(64, input_shape=(58, 1), return_sequences=False))
model.add(Activation('relu'))
...
model.fit(X_train, y_train, epochs = 1000, batch_size = 1000,validation_data=(X_test, y_test),callbacks=[change_lr]) #训练模型1000次
